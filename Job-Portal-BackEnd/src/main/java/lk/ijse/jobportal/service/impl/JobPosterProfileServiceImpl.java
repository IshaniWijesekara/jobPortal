...
public class JobPosterProfileServiceImpl implements JobPosterProfileService {
    ...
    public ArrayList<JObPosterProfileDTO> getAllProfile() {
        List<JobPosterProfile> all = jobPosterProfileRepository.findAll();

        ArrayList<JObPosterProfileDTO> getAllPosters=new ArrayList<>();

        for (JobPosterProfile jobPosterProfile:all
             ) {
            JobPoster jobPoster=jobPosterProfile.getJobPoster();
            JobPosterDTO jobPosterDTO=new JobPosterDTO(jobPoster.getUsername(),jobPoster.getCompanyname(),jobPoster.getEmail(),jobPoster.getPassword());

            JObPosterProfileDTO jObPosterProfileDTO=new JObPosterProfileDTO(
                    jobPosterProfile.getComapanyname(),
                    jobPosterProfile.getEmail(),
                    jobPosterProfile.getAddress(),
                    jobPosterProfile.getCity(),
                    jobPosterProfile.getProvince(),
                    jobPosterProfile.getContactnumber(),
                    jobPosterProfile.getCompanyBackground(),
                    jobPosterProfile.getImagepaht(),
                    jobPosterDTO
            );

            getAllPosters.add(jObPosterProfileDTO);
        }
        return getAllPosters;
    }
}
