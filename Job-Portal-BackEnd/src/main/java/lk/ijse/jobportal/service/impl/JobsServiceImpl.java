...
public class JobsServiceImpl implements JobsService {
            ...
            try {
                ...
                outputStream = new BufferedOutputStream(
                        new FileOutputStream(
                                new File("E:/Server Images/jobs", file.getOriginalFilename())));
                path = "http://localhost:8080/api/v1/profile/file?file=E:/Server Images/jobs/" + file.getOriginalFilename();
    ...
    public ArrayList<PostJobDTO> SeachJob(String categoryName) {
        ArrayList<Qulifications> allPostJob = qulificationRepository.getAllPostJob();
        ArrayList<PostJobDTO> getAll = new ArrayList<>();
        PostJobDTO postJobDTOs = new PostJobDTO();
        System.out.println("sucses");
        for (Qulifications qulifications : allPostJob
                ) {
            if (qulifications.getJobs().getCategory().equals(categoryName)) {
                Jobs jobs = qulifications.getJobs();
                JobsDTO jobsDTOl = new JobsDTO();
                System.out.println(jobs.getCategory());
                jobsDTOl.setId(jobs.getId());
